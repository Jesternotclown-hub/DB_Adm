...
Матурин Роман, студент группы ИС-22
...
![Рисунок 1](img/2.3.png)

_Рисунок 1: Выполнение резервного копирования базы данных circus с различными параметрыми_
...
![Рисунок 2](img/2.6.png)

_Рисунок 2: Файлы circus.dump/tar_
...
![Рисунок 3](img/2.7.png)

_Рисунок 3: Файл circus_dump_Aflag.sql_
...
При выполнении резервного копирования определённой схемы использовалась команда pg_dump -U postgres -d circus -n test_schema -f test_schema.sql, где -n указываем схему, для которой создаётся резервная копия, а в -f прописывается файл, в который происходит сохранение. При выполнении резервного копирования определённой таблицы из схемы public была использована команда pg_dump -U jester -d circus -t public.table_public -f backup_table_public.sql, где в -t указываем конкретную таблицу для резервного копирования, а в -f также прописываем файл (рис. 4).

![Рисунок 4](img/3.1.png)
...
В данном задании нужно было восстановить базу из резервного файла с помощью pg_restore или утилиты psql. Для востановления базы данных circus была создана новая бд circus_restore, куда будут восстанавливаться данные (рис. 5).

![Рисунок 5](img/4.1.png)

_Рисунок 5: Создание базы данных circus_restore для востановления базы данных circus_

После была выполнена команда pg_restore -U jester -d circus_restore -h localhost -p 5433 backup_circus.dump, которая позволяет восстановить структуру базы данных, схемы, таблицы и все записи из резервной копии backup_circus.dump в новую базу circus_restore.
...
В начале был создан скрипт для удобного бекапа и ротаций. (рис. 7).

![Рисунок 7](img/5.2.png)

_Рисунок 7: Добавлена команда для ежедневного резервного копирования_

Далее с помощью команды crontab -e была добавлена команда 0 6 * * * /root/backup_pg.sh >> /var/log/backup_pg.log 2>&1, которая ежедневно в 06:00 выполняет создание резервной копии базы данных с помощью утилиты pg_dump. Резервные копии сохраняются в каталоге /var/log/ с указанием даты в имени файла. После с помощью команды crontab -l была проведена проверка, что команда успешно добавлена.

![Рисунок 8](img/5.3.png)
